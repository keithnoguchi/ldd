/* SPDX-License-Identifier: GPL-2.0 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <limits.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/wait.h>
#include "kselftest.h"

struct test {
	const char	*const name;
	const char	*const path;
	unsigned int	delay_ms;
};

static void test(const struct test *restrict t)
{
	char path[PATH_MAX];
	char buf[BUFSIZ];
	FILE *fp;
	int ret;

	ret = snprintf(path, sizeof(path), "/proc/driver/%s", t->path);
	if (ret < 0)
		goto perr;
	ret = snprintf(buf, sizeof(buf), "%d\n", t->delay_ms);
	if (ret < 0)
		goto perr;
	fp = fopen(path, "w");
	if (!fp)
		goto perr;
	ret = fwrite(buf, strlen(buf), 1, fp);
	if (ret == -1)
		goto perr;
	if (fclose(fp) == -1)
		goto perr;
	fp = fopen(path, "r");
	if (!fp)
		goto perr;
	ret = fread(buf, sizeof(buf), 1, fp);
	if (ret == 0 && ferror(fp))
		goto perr;
	if (fclose(fp) == -1)
		goto perr;
	fp = fopen(path, "w");
	if (!fp)
		goto perr;
	ret = fputs("0\n", fp);
	if (ret == -1)
		goto perr;
	if (fclose(fp) == -1)
		goto perr;
	fprintf(stdout, "%s:\n%s", t->name, buf);
	exit(EXIT_SUCCESS);
perr:
	perror(t->name);
	exit(EXIT_FAILURE);
}

int main(void)
{
	const struct test *t, tests[] = {
		{
			.name		= "work queue based minimum delay",
			.path		= "jiwq",
			.delay_ms	= 0,
		},
		{
			.name		= "work queue based 1ms delay",
			.path		= "jiwq",
			.delay_ms	= 1,
		},
		{
			.name		= "work queue based 2ms delay",
			.path		= "jiwq",
			.delay_ms	= 2,
		},
		{
			.name		= "work queue based 4ms delay",
			.path		= "jiwq",
			.delay_ms	= 4,
		},
		{
			.name		= "work queue based 8ms delay",
			.path		= "jiwq",
			.delay_ms	= 8,
		},
		{
			.name		= "work queue based 16ms delay",
			.path		= "jiwq",
			.delay_ms	= 16,
		},
		{
			.name		= "work queue based 32ms delay",
			.path		= "jiwq",
			.delay_ms	= 32,
		},
		{
			.name		= "work queue based 64ms delay",
			.path		= "jiwq",
			.delay_ms	= 64,
		},
		{
			.name		= "work queue based 128ms delay",
			.path		= "jiwq",
			.delay_ms	= 128,
		},
		{
			.name		= "delay work queue based minimum delay",
			.path		= "jiwqdelay",
			.delay_ms	= 0,
		},
		{
			.name		= "delay work queue based 1ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 1,
		},
		{
			.name		= "delay work queue based 2ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 2,
		},
		{
			.name		= "delay work queue based 4ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 4,
		},
		{
			.name		= "delay work queue based 8ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 8,
		},
		{
			.name		= "delay work queue based 16ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 16,
		},
		{
			.name		= "delay work queue based 32ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 32,
		},
		{
			.name		= "delay work queue based 64ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 64,
		},
		{
			.name		= "delay work queue based 128ms delay",
			.path		= "jiwqdelay",
			.delay_ms	= 128,
		},
		{
			.name		= "single work queue based minimum delay",
			.path		= "jisinglewq",
			.delay_ms	= 0,
		},
		{
			.name		= "single work queue based 1ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 1,
		},
		{
			.name		= "single work queue based 2ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 2,
		},
		{
			.name		= "single work queue based 4ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 4,
		},
		{
			.name		= "single work queue based 8ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 8,
		},
		{
			.name		= "single work queue based 16ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 16,
		},
		{
			.name		= "single work queue based 32ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 32,
		},
		{
			.name		= "single work queue based 64ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 64,
		},
		{
			.name		= "single work queue based 128ms delay",
			.path		= "jisinglewq",
			.delay_ms	= 128,
		},
		{
			.name		= "delay single work queue based minimum delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 0,
		},
		{
			.name		= "delay single work queue based 1ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 1,
		},
		{
			.name		= "delay single work queue based 2ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 2,
		},
		{
			.name		= "delay single work queue based 4ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 4,
		},
		{
			.name		= "delay single work queue based 8ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 8,
		},
		{
			.name		= "delay single work queue based 16ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 16,
		},
		{
			.name		= "delay single work queue based 32ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 32,
		},
		{
			.name		= "delay single work queue based 64ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 64,
		},
		{
			.name		= "delay single work queue based 128ms delay",
			.path		= "jisinglewqdelay",
			.delay_ms	= 128,
		},
		{
			.name		= "shared work queue based minimum delay",
			.path		= "jisharedwq",
			.delay_ms	= 0,
		},
		{
			.name		= "shared work queue based 1ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 1,
		},
		{
			.name		= "shared work queue based 2ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 2,
		},
		{
			.name		= "shared work queue based 4ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 4,
		},
		{
			.name		= "shared work queue based 8ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 8,
		},
		{
			.name		= "shared work queue based 16ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 16,
		},
		{
			.name		= "shared work queue based 32ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 32,
		},
		{
			.name		= "shared work queue based 64ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 64,
		},
		{
			.name		= "shared work queue based 128ms delay",
			.path		= "jisharedwq",
			.delay_ms	= 128,
		},
		{
			.name		= "delay shared work queue based minimum delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 0,
		},
		{
			.name		= "delay shared work queue based 1ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 1,
		},
		{
			.name		= "delay shared work queue based 2ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 2,
		},
		{
			.name		= "delay shared work queue based 4ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 4,
		},
		{
			.name		= "delay shared work queue based 8ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 8,
		},
		{
			.name		= "delay shared work queue based 16ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 16,
		},
		{
			.name		= "delay shared work queue based 32ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 32,
		},
		{
			.name		= "delay shared work queue based 64ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 64,
		},
		{
			.name		= "delay shared work queue based 128ms delay",
			.path		= "jisharedwqdelay",
			.delay_ms	= 128,
		},
		{.name = NULL},
	};

	for (t = tests; t->name; t++) {
		int ret, status;
		pid_t pid;

		pid = fork();
		if (pid == -1)
			goto perr;
		else if (pid == 0)
			test(t);

		ret = waitpid(pid, &status, 0);
		if (ret == -1)
			goto perr;
		if (WIFSIGNALED(status)) {
			fprintf(stderr, "%s: signeled by %s\n",
				t->name, strsignal(WTERMSIG(status)));
			goto err;
		}
		if (!WIFEXITED(status)) {
			fprintf(stderr, "%s: does not exit\n",
				t->name);
			goto err;
		}
		if (WEXITSTATUS(status))
			goto err;
		ksft_inc_pass_cnt();
		continue;
perr:
		perror(t->name);
err:
		ksft_inc_fail_cnt();
	}
	if (ksft_get_fail_cnt())
		ksft_exit_fail();
	ksft_exit_pass();
}
